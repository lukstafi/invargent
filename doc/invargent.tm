...
<\body>
  ...
  disjunct is left, or we decide to pick <verbatim|LetIn> anyway (when no
  progress can be made otherwise), we return the disjunct. Otherwise we
  return the filtered disjunction. <verbatim|prepare_brs> cleans up the
  initial flattened constraints or the constraints released from
  disjunctions: it calls <verbatim|simplify_brs> on implications and
  <verbatim|flat_dsj> on each disjunction.
  ...
  <\enumerate>
    ...
    <item>registering existential return types for all <verbatim|RetType>
    first arguments and their substitution instances,
  ...
  <\eqnarray*>
    <tformat|<cwith|14|14|2|2|cell-valign|c>|<cwith|15|15|3|3|cell-valign|B>|<cwith|21|21|2|2|cell-valign|c>|<table|<row|<cell|<wide|\<exists\><wide|\<beta\>|\<bar\>><rsup|\<chi\>,k>.F<rsub|\<chi\>>|\<bar\>>>|<cell|=>|<cell|S<rsub|k>>>|<row|<cell|D<rsub|K><rsup|\<alpha\>>\<Rightarrow\>C<rsub|K><rsup|\<alpha\>>\<in\>R<rsub|k><rsup|->S<rsub|k><around*|(|\<Phi\>|)>>|<cell|=>|<cell|<with|mode|text|all
  ...
  fall-back we add there the latest answer. We redo step <math|k-1> starting
  from <math|S<rsub|k-1><around*|(|\<Phi\>|)>>. Infinite loop is avoided
  because answers already attempted are discarded. When step <math|k-1>
  cannot find a new answer, we fall back to step <math|k-2>, etc. We store
  discard lists for distinct sorts separately and we only add to the discard
  list of the sort that caused fallback. Unfortunately this means a slight
  loss of completeness, as an error in another sort might be due to bad
  choice in the sort of types. The loss is ``slight'' because of the
  dissociation step described previously. Moreover, the sort information from
  checking negative branches is likewise only approximate.
...
<\references>
  <\collection>
    ...
    <associate|CaptureEscParams|<tuple|4|?>>
