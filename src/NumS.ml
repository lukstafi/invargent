...
let abd_simple ~qcmp_v ~cmp_w
    ...
    ~bvs ~xbvs ~nonparam_vars ~discard ~validation
    ~neg_validate:_
...
let abd q ~bvs ~xbvs ?(orig_ren=empty_renaming) ?(b_of_v=empty_b_of_v)
  ...
  let ans = JCA.abd
      ...
      ~discard validation ~neg_validate ([], [], [], []) brs in
